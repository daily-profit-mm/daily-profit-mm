async function fetchLiveData() {
    try {
        ...
        // ၁။ Current Result (Live SET & Value)
        if (data.live) {
        ...
        // ၂။ Result Array ထဲက Sessions များနှင့် Modern & Internet တန်ဖိုးများကို ချိန်ညှိခြင်း
        if (data.result && Array.isArray(data.result)) {
            data.result.forEach(item => {
                if (item.open_time === "11:00:00") {
                    ...
                    // 09:30 AM (Modern & Internet) အတွက် data ပါလာပါက ဤနေရာတွင် ထည့်သွင်းနိုင်ပါသည်
                ...
                } else if (item.open_time === "12:00:00") {
                    ...
                    // 02:00 PM (Modern & Internet) အတွက်
